...
const SUBTASK_ID: &str = "FNP-P2C-002-H";
...
struct Packet002OptimizationReport {
    schema_version: u8,
    packet_id: String,
    subtask_id: String,
...
struct DTypeFixtureCase {
    id: String,
}

fn main() {
    if let Err(err) = run() {
        eprintln!("generate_packet002_optimization_report failed: {err}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), String> {
    ...
    let case_ids = load_dtype_case_ids(&repo_root)?;
    if case_ids.is_empty() {
        return Err("packet002 dtype fixture corpus is empty".to_string());
    }

    let scaled_case_ids = expand_case_ids(&case_ids, 256);
    ...
    let report = Packet002OptimizationReport {
        schema_version: 1,
        ...
        reproducibility_command:
            "cargo run -p fnp-conformance --bin generate_packet002_optimization_report"
                .to_string(),
        lever: LeverSummary {
            id: "P2C002-H-LEVER-001".to_string(),
            description:
                "Use precomputed packet-002 dtype fixture ID maps for workflow-step dispatch"
                    .to_string(),
            rationale: "Packet-002 workflow replay repeatedly resolves dtype differential and adversarial fixture IDs for strict/hardened scenario steps; replacing repeated linear scans with precomputed HashMap lookups removes O(n) per-step lookup overhead while preserving deterministic fixture resolution semantics."
                .to_string(),
            risk_note:
                "Optimization only changes fixture-ID lookup strategy in workflow dispatch; fixture payload selection, step ordering, and reason-code outcomes remain unchanged."
                    .to_string(),
            rollback_command:
...
fn parse_output_path(repo_root: &Path) -> Result<PathBuf, String> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--help" | "-h" => {
                println!(
                    "Usage: cargo run -p fnp-conformance --bin generate_packet002_optimization_report -- [--output-path <path>]"
                );
                std::process::exit(0);
            }
    ...
    Ok(output_path.unwrap_or_else(|| {
        repo_root.join("artifacts/phase2c/FNP-P2C-002/optimization_profile_report.json")
    }))
}

fn load_dtype_case_ids(repo_root: &Path) -> Result<Vec<String>, String> {
    let dtype_root = repo_root.join("crates/fnp-conformance/fixtures/packet002_dtype");
    let differential_path = dtype_root.join("dtype_differential_cases.json");
    let adversarial_path = dtype_root.join("dtype_adversarial_cases.json");

    let mut case_ids = Vec::new();
    case_ids.extend(load_case_ids_from_path(&differential_path)?);
    case_ids.extend(load_case_ids_from_path(&adversarial_path)?);
    Ok(case_ids)
}

fn load_case_ids_from_path(path: &Path) -> Result<Vec<String>, String> {
    let raw = fs::read_to_string(path)
        .map_err(|err| format!("failed reading {}: {err}", path.display()))?;
    let cases: Vec<DTypeFixtureCase> = serde_json::from_str(&raw)
...
fn run_isomorphism_checks(
    ...
    let missing_id = "dtype::nonexistent::fixture";
